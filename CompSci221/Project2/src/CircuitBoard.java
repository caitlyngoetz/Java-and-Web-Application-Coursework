...
public class CircuitBoard {
	...
	public CircuitBoard(String filename) throws FileNotFoundException {
		...
		//FileNotFoundException if scanner can't read file.
		...
		gridX = fileScan.next();
		ROWS = Integer.parseInt(gridX);
		gridY = fileScan.next();
		COLS = Integer.parseInt(gridY);
		...
		board = new char[ROWS][COLS];
			...
			for(int row = 0; row < board.length; row++){
				for(int col = 0; col < board[row].length; col++){
					if(fileScan.hasNext()){
					String thisChar = fileScan.next();
					board[row][col] = thisChar.charAt(0);
